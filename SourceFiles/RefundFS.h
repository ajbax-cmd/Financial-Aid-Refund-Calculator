...
//class that creates refund objects for the Fall and Spring semesters
...
{
private:
	int const FOUR = 4;					// Number of disbursement periods
	int size;							// Number of classes student is taking
	int Pell;							// Pell Grant total
	int FTPell;							// Pell Grant amount if student was FT
	int PellPot[4] = { 0 };				// Array holding Pell Grant amounts for each POT disbursement
	double SubLoanPot[4] = { 0 };		// Array holding Sub loan amounts for each POT disbursement
	double UnsubLoanPot[4] = { 0 };		// Array holding Unsub loan amounts for each POT disbursement
	double disb[4] = { 0 }; 			// Array to conatain the number of credits for each disbursement period
	double Refund[4] = { 0 };			// Array to contain refund amounts for each disbursement period
